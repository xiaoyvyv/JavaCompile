    ...
    public Void visitNewClass(NewClassTree node, Void unused) {
        ...
        if (node.getClassBody() != null) {
            List<Op> ops = visitModifiers(node.getClassBody().getModifiers(),
                    Direction.HORIZONTAL, Optional.absent());
            builder.addAll(ops);
        }
        ...
        builder.open(ZERO);
        {
            ...
            if (!node.getParameters().isEmpty()/* || node.getReceiverParameter() != null*/) {
                ...
                visitFormals(/*Optional.fromNullable(*//*node.getReceiverParameter())*/
                        Optional.absent(), node.getParameters());
            }
            token(")");
            if (dims != null) {
                maybeAddDims(dims);
...
/*
   ...
   /* @Override
    ...
    public void visitClassDeclaration(ClassTree node) {
        sync(node);
        List<Op> breaks =
                visitModifiers(node.getModifiers(), Direction.VERTICAL, Optional.absent());
...
//        visitAnnotations(node.getAnnotations(), BreakOrNot.NO, BreakOrNot.YES);
    ...
    private void visitStatements(List<? extends StatementTree> statements) {
        boolean first = true;
        PeekingIterator<StatementTree> it =
                Iterators.peekingIterator(statements.iterator());
        dropEmptyDeclarations();
        while (it.hasNext()) {
    ...
    private void visitUnionType(VariableTree declaration) {
        ...
        visitAndBreakModifiers(
                declaration.getModifiers(), Direction.HORIZONTAL, Optional.absent());
